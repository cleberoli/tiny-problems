...
class EnumerationTree:
    ...
    def make_tree(self):
        root_node = self.__add_node(0, list(self.polytope.vertices.keys()), list(self.polytope.H.keys()), Region())
        self.queue.append(root_node)
        self.bfs()
        print(self.graph.nodes.data())
        print(self.graph.edges.data())

    def __add_node(self, height: int, solutions: List[int], hyperplanes: List[int], region: Region) -> int:
        self.graph.add_node(self.__get_next_node(), height=height, solutions=solutions, hyperplanes=hyperplanes, region=region)
    ...
    def bfs(self):
        ...
        while len(self.queue) > 0:
            ...
            height, solutions, hyperplanes, region = node['height'], node['solutions'].copy(), node['hyperplanes'].copy(), deepcopy(node['region'])
            ...
            # print(solutions, left_solutions, right_solutions)
            ...
            left_node = self.__add_node(height + 1, left_solutions, children_hyperplanes, left_region)
            self.graph.add_edge(parent_node, left_node, direction='left')

            if len(left_solutions) > 1:
                self.queue.append(left_node)

            right_node = self.__add_node(height + 1, right_solutions, children_hyperplanes, right_region)
