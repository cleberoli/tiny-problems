...
def test_cut_polytope_5():
    ...
    assert cut5.size == 16
    assert cut5.n == 5
    assert len(cut5.vertices) == 16
    assert cut5.instance.get_solution_dict() == CutInstance(n=5, origin=True, save=False).get_solution_dict()


def test_cut_polytope_6():
    cut6 = CutPolytope(6, False, False)
    ...
    assert cut6.instance.get_solution_dict() == CutInstance(n=6, origin=False, save=False).get_solution_dict()
